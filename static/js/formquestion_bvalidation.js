    ...
    Array.prototype.slice.call(forms).forEach(function (form) {
        form.addEventListener('submit', function (event) {
            if (!form.checkValidity()) {
                 alertaDanger('Llenar todos los campos correctamente.');
                event.preventDefault();
                event.stopPropagation();
                form.classList.add('was-validated');
            } else {
                // Validación exitosa, enviar el formulario por Ajax
                event.preventDefault();
                event.stopPropagation();

                // Verificar el campo de selección personalizado
                var selectField = form.querySelector('.selectpicker');
                if (selectField && !selectField.value) {
                    selectField.classList.add('is-invalid');
                    selectField.parentNode.classList.add('has-danger');
                    selectField.nextElementSibling.style.display = 'block';
                    event.preventDefault();
                    event.stopPropagation();
                    return;
                }
